import { CheckSongResponse, CheckSongRequest, BlockArtistRequest, BlockTrackRequest, MessageType } from '@/utils/types';
import { STORAGE_KEY_AUTO_SKIP } from '@/utils/constants';
...
function skipTrack(expectedTitle?: string) {
    if (expectedTitle) {
        const info = getSongInfo();
        if (!info || info.title !== expectedTitle) {
            console.log(`[YTM No Slop] Aborting skip: title mismatch. Expected "${expectedTitle}", found "${info?.title}"`);
            return;
        }
    }
...
function performDownvoteAndSkip(songTitle: string, reason: string) {
...
async function checkAndSkip() {
    ...
    try {
        ...
        if (response.shouldSkip) {
            ...
            performDownvoteAndSkip(song.title, response.reason);
        }
    } catch (e) {
...
function injectButtons() {
    ...
    if (targetParent) {
        ...
        container.appendChild(createButton("Artist", async () => {
            const song = getSongInfo();
            if (song && song.artistId) {
                ...
                performDownvoteAndSkip(song.title, "Manual Artist Block");
            } else {
                console.warn("[YTM No Slop] No Artist ID found to block.");
            }
        }));

        container.appendChild(createButton("Song", async () => {
            const song = getSongInfo();
            if (song) {
                await browser.runtime.sendMessage({
                    ...
                    title: song.title,
                    artistName: song.artistName
                } as BlockTrackRequest);

                performDownvoteAndSkip(song.title, "Manual Track Block");
            }
        }));
